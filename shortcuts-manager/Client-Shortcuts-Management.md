...
| `domain`        | ✔        | Windows domain for `runas /user:`. |
| `user`          | ✔        | Windows username for `runas /user:`. |
...
The generated shortcut is equivalent to writing, in the shortcut's *Target*
field:

```
C:\Windows\System32\runas.exe /user:DOMAINA\userA "\"C:\...\Ssms.exe\" \"C:\Projects\CustomerA\CustomerA.ssmssln\""
```

The inner `\"…\"` escaping is required because `runas.exe` takes the whole
program+arguments block as a single quoted string and passes it verbatim to
`CreateProcess`.
